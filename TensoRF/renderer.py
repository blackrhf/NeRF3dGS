...
# @torch.no_grad()
# def generate_orbit_path(c2w, num_frames=60, radius=1.2, elevation=30):
#     """
#     生成相机环绕轨迹
#     c2w: 参考相机位姿
#     num_frames: 总帧数
...
#     import math
#     poses = []
#     for theta in np.linspace(0, 2*math.pi, num_frames):
#         theta = theta + math.pi/4  # 起始角度偏移
#         cam_origin = np.array([
...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, device='cuda'):
    ...
    # # 添加进度条
    # pbar = tqdm(total=N_rays_all, desc="渲染光线", leave=False, mininterval=0.5)
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        #  # 更新进度条
        # pbar.update(rays_chunk.shape[0])
    # pbar.close()
...
def evaluation(test_dataset,tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    imageio.mimwrite(f'{savePath}/{prtx}video.mp4', np.stack(rgb_maps), fps=30, quality=10) #10
